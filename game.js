...
// Edge-to-edge distance between two sticks (centerline dist minus half-thickness each)
function stickEdgeDist(x1, y1, w1, a1, x2, y2, w2, a2) {
  ...
  return cl - STICK_H; // subtract full thickness (6px each side)
}
...
function generateObstacles() {
  const W = canvas.width, H = canvas.height;
  // Each entry: { x, y, w, angle } for distance checking
  const placed = [];
  ...
  for (let i = 0; i < OBSTACLE_COUNT; i++) {
    tryPlace(60, () => {
      ...
      // Constrain position so both endpoints stay within canvas
      const hw     = w / 2;
      ...
      // Check edge-to-edge distance against all previously placed obstacles
...
function applyGyroForce() {
  ...
  // Negate x: tilting right gives negative accelerationIncludingGravity.x on most devices
